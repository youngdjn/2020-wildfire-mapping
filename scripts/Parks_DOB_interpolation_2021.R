...
for (xx in 1:length(fire.list)) {
	...
	if (nrow(fire.hotspots) > 0 ) {
		...
		# New:
		dir.create(datadir(paste0("fire_progressions_2021/", fire)), recursive=TRUE)
		file.name <- datadir(paste0("fire_progressions_2021/",fire,"/", fire,"_hotspots.shp"))
		st_write(fire.hotspots, file.name, delete_layer=TRUE)
		# End DYoung modified
...
for (xx in 1:length(fire.list)) {
	...
	if (dir.exists(datadir(paste0("fire_progressions_2021/", fire)))) { # <- DYoung mod using new data directory reference
		...
		fire.hotspots <- st_read(datadir(paste0("fire_progressions_2021/",fire,"/", fire,"_hotspots.shp"))) # <- DYoung mod to use new data directory reference
...
for (xx in 1:length(fire.list)) {
	...
	if (file.exists(datadir(paste0("fire_progressions/",fire,"/", fire,"_dob.temp.tif")))) {
	  ...
	  # V DYoung mod to new data folder reference
		modeled.dob.raster <- raster(datadir(paste0("fire_progressions_2021/",fire,"/", fire,"_dob.temp.tif")))
		...
		if(nrow(dob.df) == 0) {
		  writeRaster(modeled.dob.raster, datadir(paste0("fire_progressions_2021/",fire,"/", fire,"_dob.tif")), format="GTiff", options=c("COMPRESS=LZW", "TFW=YES"), datatype='INT2U', overwrite=T)
		  next()
		}
